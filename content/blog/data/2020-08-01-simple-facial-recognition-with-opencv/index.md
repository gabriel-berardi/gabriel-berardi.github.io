...
aliases: ["/data/2020-08-01-simple-facial-recognition-with-opencv/"]
---

Have you ever seen some cool applications of computer vision tools, like this the one below?

Perhaps your phone's camera can autofocus on faces, or maybe you have uploaded a photo on a social media platform and it automatically recognized the person on the image?
...
![facial recognition demo animation](images/facial-recognition-demo-animation.gif)
...
{{< image src="images/people-photo-for-face-counting.jpeg" alt="people photo for face counting" width="80%" >}}
...
plt.show()
```
![grayscale image for face detection](images/grayscale-image-for-face-detection.png)

The reason why we remove the colors of the image has something to do with how computer vision algorithms work. Here is a brief explanation for this:

A colorless image can be represented as a matrix of values between 0 and 255 for each pixel in the image, in which case 0 represents black and 255 represents 
white and all values in between represent a certain shade of gray. However, a colorful image can only be represented as a three-dimensional matrix or three 
matrices stacked on top of each other, with each matrix containing values of 0 to 255 for the red, green and blue component of the color of a pixel. Computer 
vision then uses these values as numerical inputs to algorithms to perform classification tasks.

Now, some computer vision tasks might certainly perform much better when using colorful images. One might imagine that an algorithm can distinguish between an 
apple and a peach more easily when the images contain the color-information. Other tasks, such as facial recognition, do not require colors and can perform much 
...
![haarcascades xml files list](images/haarcascades-xml-files-list.png)

You can also find these on OpenCV's [GitHub page](https://github.com/opencv/opencv).
...
Then we use the `.detectMultiScale` method and pass three arguments: our gray-scale image, `scaleFactor` and `minNeighbors`. The `scaleFactor` parameter specifies how much the 
image size is reduced at each image scale. The `minNeighbors` argument specifies how many neighbors each candidate rectangle should have to retain it. In other words, the `minNeighbors` parameter will affect the quality of detected faces. A high value results in fewer detections but with higher quality.
...
print(faces)
```
![detected faces coordinates output](images/detected-faces-coordinates-output.png)
...
plt.imshow(img)
```
![detected faces with rectangles result](images/detected-faces-with-rectangles-result.png)
...
{{< image src="images/facial-recognition-result-people-group-photo-1.png" alt="facial recognition result people group photo 1" width="50%" >}}
{{< image src="images/facial-recognition-result-people-group-photo-2.png" alt="facial recognition result people group photo 2" width="50%" >}}
{{< image src="images/facial-recognition-result-people-group-photo-3.png" alt="facial recognition result people group photo 3" width="50%" >}}
{{< image src="images/facial-recognition-result-people-group-photo-4.png" alt="facial recognition result people group photo 4" width="50%" >}}
...
{{< image src="images/facial-recognition-failure-example-1.png" alt="facial recognition failure example 1" width="50%" >}}
{{< image src="images/facial-recognition-failure-example-2.png" alt="facial recognition failure example 2" width="50%" >}}
{{< image src="images/facial-recognition-failure-example-3.png" alt="facial recognition failure example 3" width="50%" >}}
{{< image src="images/facial-recognition-failure-example-4.png" alt="facial recognition failure example 4" width="50%" >}}

That's it for this post. In another post, I am going to demonstrate how OpenCV can be used for object recognition in videos.

## [Full Code on GitHub](https://gist.github.com/gabriel-berardi/3e4aeeebbe0b27eb030e8c84738ace9a)
