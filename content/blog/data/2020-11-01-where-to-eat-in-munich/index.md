...
date: 2020-11-01
categories: ["Data"]
tags: ["python", "web-scraping", "data-visualization", "data-quality"]
aliases: ["/data/2020-11-01-where-to-eat-in-munich/"]
---
...
Next, we specify the base URL, as well as the first URL extension of the website that we want to scrape. The main idea is that we are going to iterate over many pages containing information about the restaurants, where the base URL stays the same and only the URL extension changes, for example like this:
...
Now we will collect all the URL extensions for each individual restaurant in a list. We do this by looking for all URLs referring to the respective CSS class and appending them to the list. Then we look for the "Next Page" button at the end of the website and concatenate the new URL extension to our base URL.

When the last page has been reached, our script won't find the "Next Page" button and will end the loop. We then have a list containing all URL extensions for 3,821 restaurants in Munich. This step of the scraping took around 30 minutes.
...
Sometimes, the information we want might not be available in the form we expect. Therefore, we should use some `try/except` statements to avoid our code from breaking. Since I added a random time stop of 1, 2 or 3 seconds between all of my requests, this piece of code took almost 4 hours to finish! Therefore, I suggest using the `pickle` package to save results after every 1000th restaurant.
...
The last thing I did was to generate a data frame with the data and save it to a CSV file.
...
![scraped restaurants dataset](images/scraped-restaurants-dataset.png)
...
- Lastly, the "number_reviews" and the "rating" columns have a wrong data type and should be converted to integers and floats respectively.
...
![dataset info with null values](images/dataset-info-with-null-values.png)
...
Proceeding, we take care of the wrong data types in the "number_reviews" and the "rating" column.
...
![cleaned restaurants dataset](images/cleaned-restaurants-dataset.png)

Finally, we can visualize this data on a map.

## Visualizing the Data

The first thing we obviously need to do is to load the cleaned data set. Next, we are going to group it by the area code and taking the mean of the other variables. 

Because we are interested in the total number of reviews for each area code, we define this column specifically. Then we make sure that the area code is a string data type and rearrange the columns.
...
![grouped by area dataframe](images/grouped-by-area-dataframe.png)

Since we want to produce a map, we are going to need some geospatial information about each area code. After quite some research, I found a way to add a polygon to each area code. I downloaded the "plz-gebiete.shp" shapefile which contains geometrical polygons for most area codes in Germany from https://www.suche-postleitzahl.org/downloads. I load this data into a `pandas` data frame, drop any unnecessary column and then filter out the data for area codes starting with "8" 
...
![area shape dataframe with geometry](images/area-shape-dataframe-with-geometry.png)
...
Next, we need to think about our visualization a bit more. I would like to have a map of Munich that colors different areas according to the quality of the restaurants in that area. The question is how we want to define "quality" in this context.
...
You have probably come up with some heuristics to help you decide what movie to watch in such a setting. If you are like me, you would probably go with movie A, since an average rating of 8 with 1.000 reviews is more likely to be a good movie than movie B with only 50 reviews. Movie C would be out of question, because 2 reviews is simply to few and the rating could even be fake.

The importance we assign to the average rating and the number of reviews respectively will always be subjective, but we should at least try to come up with a way to incorporate both of these aspects into our measurement of "quality" for the restaurants in Munich.
...
algorithm that takes in both the average rating and the number of reviews and outputs a score. I have adapted the algorithm to our current setting and it looks like this:

$$ \text{score} = 0.5 \times p + 2.5 \times \left(1 - e^{-\frac{q}{Q}}\right) $$    

Where p is the average rating, q is the number of reviews and Q is the median of the number of reviews. Note that in the original algorithm, Marc Bogaerts proposed to set Q equal to a number that we would consider "moderate". In my opinion the median is a good approach to a "moderate" value, for our situation.

- Let's take a look at the first half of the formula:

    $ 0.5 \times p $
    
    This tells us that 50% of the final score is determined by the average rating.

- Now let's look at the second half:

    $ 2.5 \times \left(1 - e^{-\frac{q}{Q}}\right) $

    The expression $ e^{-\frac{q}{Q}} $ can take values between 0 and 1.
    
    Suppose the number of ratings is equal to 1 and the median number of reviews is 1000. e^(-1/1000) is equal to 0.999, which lead the term inside brackets to be slightly over 0 and the second half of the expression to be almost 0 as well. Hence, such a low number of ratings will punish the score, by basically taking only half of the average rating. For more information on the formulae, I suggest reading the linked post.
...
![final dataframe with score column](images/final-dataframe-with-score-column.png)
...
![map with colored polygons](images/map-with-colored-polygons.png)

In order to make the map clearer, I simply used this map as the background for the plot:

![munich map background](images/munich-map-background.png)
...
![final visualization with annotations](images/final-visualization-with-annotations.png)

And that's the final visualization. As you can see, the polygons do not match up perfectly with the underlying map on the borders, but this is fine for our little project here.
...
## [Full Code on GitHub](https://gist.github.com/gabriel-berardi/743fbcaf874badce9469e1ad41591bcb)
