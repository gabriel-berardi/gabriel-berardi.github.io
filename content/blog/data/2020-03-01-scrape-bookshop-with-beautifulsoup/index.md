...
aliases: ["/data/2020-03-01-scrape-bookshop-with-beautifulsoup/"]
---
...
There has been a lot of discussion going on about the legality and ethics of Web Scraping, which I do not want to get into in this article. You can find a lot of information on this online.

Some websites simply do not want you to scrape them. A good practice is to consult the `robots.txt` file before you scrape a website. This is basically a guideline by the website, which states what automated actions they allow or disallow. You can see this file by adding `/robots.txt` to any website's URL (e.g. [https://www.ikea.com/robots.txt](https://www.ikea.com/robots.txt)).
...
![book shop website homepage screenshot](images/bookshop-website-homepage.png)

There are 1,000 books on this website, and our goal is to get data for all of them!

## Importing the Required Packages

As always, we need some packages to perform the task at hand.

- We will use [pandas](https://pandas.pydata.org/) to assemble the extracted data in a data frame, which we will output as a CSV or Excel file at the end.
- We use the [requests](https://requests.readthedocs.io/en/latest/) module to get the HTML code from the website.
- The [time](https://docs.python.org/3/library/time.html) module is needed to add a short break in between each request.
- [BeautifulSoup](https://beautiful-soup-4.readthedocs.io/en/latest/) is needed to perform the actual scraping.
- Finally, we need the urljoin function from the [urllib.parse](https://docs.python.org/3/library/urllib.parse.html) module in order to correctly scrape the url of the images.
...
- Then, we have the `main` loop. `doc.select('.next')` will be a `True` boolean value as long as there is a "Next" button on the webpage that is currently being scraped. As soon as we have reached the last page, the while loop will stop.
...
![jupyter notebook showing URL being scraped](images/jupyter-notebook-scraping-url.png)
...
![pandas data frame with scraped book data](images/jupyter-notebook-scraped-data-frame.png)

... or in Excel:

![excel spreadsheet with scraped book data](images/excel-scraped-data-output.png)

After you have successfully extracted data from a website, you might then proceed to analyze this data and derive some meaningful insights from it. 

But that's for homework!

## [Full Code on GitHub](https://gist.github.com/gabriel-berardi/3fc044964ed5806a78fc0a1d413afdb6)
